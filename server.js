...
async function generateCronTweet(commits, user) {
    try {
        ...
        // Extract repository name from the first commit (they should all be from the same repo)
        const firstCommit = commits[0];
        const repoNameMatch = firstCommit.match(/\((.*?)\)$/);
        const repoName = repoNameMatch ? repoNameMatch[1] : 'project';
        
        // Create a summary of commits (without the repo name in parentheses)
        const commitSummary = commits.map((commit, index) => {
            const cleanCommit = commit.replace(/\s*\([^)]*\)$/, ''); // Remove repo name from end
            return `${index + 1}. ${cleanCommit}`;
        }).join('\n');
...
6.  **Engagement & Authenticity:**
     * **Include project name if needed** (${repoName}) in tweet to provide context about which project the updates are for.
